...
def copy_file(source_file, source_folder, destination_folder):
    ...
    """
    shutil.copy(source_folder / source_file.name,\
...
def folder_buildr(folder, make_dir=False, img_dir = 'img', masks_dir = 'masks'):
    ...
    if make_dir: folder_makedirs(image_folder, masks_folder)
...
def kfold_folder_buildr(folder, sub_folder, fold, split_set, make_dir=False):
    ...
    if make_dir: folder_makedirs(image, masks)
    return image, masks
...
def write_image_to_subimages(img,imname,target_folder,target_width,target_height,multiply=1,dpi = None):
    """ 
    ...
    for w in range(N_width+1):
        ...
        for h in range(N_height+1):
            ...
            name_out = '{}_rowstart_{}_rowend_{}_colstart_{}_colend_{}.png'.format(imname,row_start,row_end,col_start,col_end)
