...
def unet_kfold_prediction(
    k_fold, LR, EPOCH, batch, weights, df_results, out_folder, input_dataset
):
    ...
    for fold in range(1, k_fold + 1):
        ...
        folder_fold = input_dataset / "05_K_fold_cv_rgb" / f"fold_{fold}" / SUBSET
        ...
        with strategy.scope():
            ...
            model.compile(
                loss=dice_loss, optimizer=opt, metrics=metrics, run_eagerly=True
            )
        ...
        outfile = out_folder / Path(
            "LR_{}_EPOCHS_{}_batch_{}_fold_{}.png".format(LR, EPOCH, batch, fold)
        )
