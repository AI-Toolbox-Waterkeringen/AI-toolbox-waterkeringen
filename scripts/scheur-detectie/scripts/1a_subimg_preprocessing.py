...
for img_file, mask_file in zip(sorted(img.glob('*.png')), sorted(masks.glob('*.png'))):
    ...
    write_image_to_subimages(img,img_file.stem,sub_img,\
        target_width,target_height,multiply = 1,dpi = None)
    write_image_to_subimages(mask,mask_file.stem,sub_masks,\
        target_width,target_height,multiply = 255,dpi = None)