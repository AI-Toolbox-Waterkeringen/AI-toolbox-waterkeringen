...
class Augmentation:
    ...
    def geotransform(self, image, mask):
        transform = A.Compose(
            [
                A.augmentations.transforms.Flip(p=0.5),
        ...
        for img in sorted(image_folder_in.glob(extentie)):
            ...
            while not (processed == N_transform):
                ...
                if self.check_unique_augment(image_transform, list_hist):
                    ...
                    imname_augmented = image_folder_out / (
                        img.stem + "_" + str(processed) + img.suffix
                    )
                    imname_mask = mask_folder_out / (
                        img.stem + "_" + str(processed) + img.suffix
                    )
