...
for fold in range(1, k_fold + 1):
    ...
    for split in splits:
        ...
        data_folder_img, data_folder_mask = kfold_folder_buildr(
            data_kfolds_fil, sub_filter, fold, split
        )
        out_geotf_img, out_geotf_mask = kfold_folder_buildr(
            data_kfolds_geo, sub_geo, fold, split, make_dir=True
        )
        out_rgbtf_img, out_rgbtf_mask = kfold_folder_buildr(
            data_kfolds_rgb, sub_rgb, fold, split, make_dir=True
        )

        print("Start geo augmentation")
        Augment.apply_transform(
            Augment.geotransform,
            data_folder_img,
            data_folder_mask,
            out_geotf_img,
            out_geotf_mask,
