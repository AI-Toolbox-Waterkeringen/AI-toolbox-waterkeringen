...
(
    (train_x_kfold, train_y_kfold),
    (valid_x_kfold, valid_y_kfold),
    (test_x_kfold, test_y_kfold),
) = load_data_cross(data_filter, k_fold, split)

for fold in range(1, k_fold + 1):
    print("Processing fold {}".format(fold))

    img_train, mask_train = kfold_folder_buildr(
        data_kfolds, sub_filter, fold, "train", make_dir=True
    )
    img_test, mask_test = kfold_folder_buildr(
        data_kfolds, sub_filter, fold, "test", make_dir=True
    )
    img_val, mask_val = kfold_folder_buildr(
        data_kfolds, sub_filter, fold, "val", make_dir=True
    )
